...
export function EditPastWorkout({
    workout,
    className
}: {
    workout: PastWorkout,
    ...
    const [exercices, setExercices] = useState<Exercice[]>(workout.exercices.map(ex => ({
        ...ex,
        sets: ex.sets.map((s, idx) => ({ ...s, id: idx }))
    })))
    const [workoutName, setWorkoutName] = useState(workout.title)
    const [workoutDate, setWorkoutDate] = useState(new Date(workout.date).toISOString().split('T')[0])
    const [difficulty, setDifficulty] = useState<1 | 2 | 3 | 4 | 5>(workout.difficulty)
    const [isSaving, setIsSaving] = useState(false)
    const { updateWorkout, deleteWorkout } = useWorkoutActions()
                    ...
                    <form onSubmit={handleSave} className={"space-y-4 mx-auto w-full max-w-[calc(100vw-5.25rem)] overflow-y-auto sm:max-w-[462px] lg:max-w-[718px] flex flex-col justify-between"}>
                                                        ...
                                                        <TableHead className={"flex justify-between items-center"}>
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => {
                                                                    ...
                                                                    const nextId = currentSets.length > 0 ? Math.max(...currentSets.map(s => s.id || 0)) + 1 : 0;
                                                        ...
                                                        <TableRow key={setIndex} className={"group/row"}>
                                                            <TableCell className="flex justify-between items-center">
                                                                ...
                                                                <Button
                                                                    ...
                                                                    onClick={() => {
                                                                        const newExercices = [...exercices];
                                                                        newExercices[index].sets = exercice.sets.filter((_, i) => i !== setIndex);
                                                                        setExercices(newExercices);
                                                                    }}
                                                                >
                                                                    <Minus className={"size-4"}/>
                        ...
                        <footer className={"flex justify-end items-center gap-2"}>
                            <Button
                                ...
                                disabled={isSaving}
                                onClick={() => {
                                    setExercices(workout.exercices.map(ex => ({
                                        ...ex,
                                        sets: ex.sets.map((s, idx) => ({ ...s, id: idx }))
                                    })))
