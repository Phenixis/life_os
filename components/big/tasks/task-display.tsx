  ...
  const handleUpdateDueToday = () => {
    ...
    updateTaskMutation.mutate({
      ...
      data: {
        ...
        project: task.project_id ? { id: task.project_id, title: task.project?.title ?? '' } : { id: -1, title: '' },
      ...
      <ContextMenuTrigger asChild>
        ...
        >
                    ...
                    >
                      <div
                        className={`absolute inset-0 w-1/2 h-1/2 z-20 m-auto duration-300 rounded-xs ${
                          optimisticState
                            ? 'lg:group-hover/Clickable:bg-background'
                            : 'lg:group-hover/Clickable:bg-primary'
                        }`}
                      />
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      <Tooltip tooltip="Delete task">
                        <TrashIcon className="min-w-4 max-w-4 min-h-6 max-h-6 cursor-pointer" />
                      </Tooltip>
                    </div>
                  </div>
                </div>
          ...
          <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
            <DialogContent className="sm:max-w-sm md:max-w-md lg:max-w-xl" maxHeight="max-h-64 lg:max-h-52">
              <DialogHeader>
          ...
          quickActions.map((action, index) => {
            ...
            return (
              <React.Fragment key={index}>
                ...
                <ContextMenuItem onClick={() => action.action(task)} variant={action.variant || 'default'}>
                  {action.label}
                </ContextMenuItem>
              </React.Fragment>
            );
